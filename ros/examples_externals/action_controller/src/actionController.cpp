  ...
  Controller(ros::NodeHandle& n)
      : node_(n),
        action_server_(node_,
                       ...
                       "multi_dof_follow_joint_trajectory",
    ...
    sub_topic = node_.subscribe<geometry_msgs::PoseStamped>(
        "/actual_pose", 1,
        boost::bind(&Controller::OnPoseStampedMessage, this, _1));
 ...
 private:
  ...
  void goalCB(GoalHandle gh) {
    ...
    if (pthread_create(&trajectoryExecutor, NULL, threadWrapper, this) == 0) {
      ...
      ROS_INFO(
          "*****actionController: Trajectory execution thread creation failed");
    }
  }

  void OnPoseStampedMessage(
      const geometry_msgs::PoseStampedConstPtr& pposestamped) {
  ...
  bool publishTranslationComand(
      geometry_msgs::Transform_<std::allocator<void>> punto, bool anyway,
      double dt) {
    ...
    if (anyway || cmd.linear.x >= 0.05 || cmd.linear.y >= 0.05 ||
        cmd.linear.z >= 0.05) {
      printPositionInfo();
      printCmdInfo();
      ...
      // Wait until we're near the destination
      {
        ...
        while (ros::Time::now() < timestamp_timeout) {
          ...
          if (ros::Time::now() > timestamp_timeout) {
            ROS_ERROR(
                "*****actionController: Timeout moving robot to target "
                "position");
            return (false);
          }
  ...
  void publishRotationComand(
      geometry_msgs::Transform_<std::allocator<void>> punto, bool start) {
    ...
    double sleep = cmd.angular.z *
                   3.0;  // tempo necessario a tornare nella giusta orientazione
  ...
  void printPositionInfo() {
    ROS_INFO_STREAM(
        "Start Position: ["
        << lastPosition.translation.x << ", " << lastPosition.translation.y
  ...
  void printCmdInfo() {
    ROS_INFO_STREAM("cmd to execute: "
                    << "x:" << cmd.linear.x << " y: " << cmd.linear.y
                    << " z: " << cmd.linear.z << " rX: " << cmd.angular.x
                    << " rY: " << cmd.angular.y << " rZ: " << cmd.angular.z);
  }
};
