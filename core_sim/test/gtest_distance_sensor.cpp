...
class Robot {
 public:
  static DistanceSensor MakeDistanceSensor(const std::string& id,
                                           bool is_enabled,
    ...
    return DistanceSensor(id, is_enabled, parent_link, logger,
                          TopicManager(logger), parent_topic_path,
                          ServiceManager(logger), StateManager(logger));
  }

  static void LoadDistanceSensor(DistanceSensor& DistanceSensor,
...
TEST(DistanceSensor, SetsDistanceSensorID) {
  auto DistanceSensor_json =
      projectairsim::Robot::GetBasicDistanceSensorConfig();
  ...
  auto DistanceSensor =
      projectairsim::Robot::MakeDistanceSensor(id, is_enabled, parent_link);
...
TEST(DistanceSensor, SetsSensorType) {
  ...
  auto DistanceSensor =
      projectairsim::Robot::MakeDistanceSensor(id, is_enabled, parent_link);
  EXPECT_EQ(DistanceSensor.GetType(),
            projectairsim::SensorType::kDistanceSensor);
}
...
TEST(DistanceSensor, SetsIsEnabled) {
  ...
  is_enabled = true;
  DistanceSensor = projectairsim::Robot::MakeDistanceSensor(
      "TestDistanceSensor", is_enabled, parent_link);
...
TEST(DistanceSensor, DistanceSensorSetting) {
  ...
  EXPECT_EQ(DistanceSensor.IsLoaded(), false);
  projectairsim::Robot::LoadDistanceSensor(DistanceSensor, DistanceSensor_json);
  const auto& DistanceSensor_settings =
      DistanceSensor.GetDistanceSensorSettings();
