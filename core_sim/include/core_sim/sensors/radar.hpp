...
struct RadarSettings {
  ...
  float data_latency = 0.0f;         // sec
  float rcs_adjust_factor = 1.0f;
  float beam_radius = 0.0f;          // m
  float range_noise_stddev = 0.0f;       // m
  float azimuth_noise_stddev = 0.0f;     // rad
  float elevation_noise_stddev = 0.0f;   // rad
  float velocity_noise_stddev = 0.0f;    // m/s
  float point_dropout_probability = 0.0f;
  bool continuous_coverage = false;
  int points_per_frame = 0;
  int max_points_per_frame = 0;
  bool quantize_azimuth = false;
  bool quantize_elevation = false;
  bool quantize_range = false;
  bool quantize_velocity = false;
  bool cluster_by_resolution = false;
