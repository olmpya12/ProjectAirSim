...
class Trajectory::Impl : public Component {
 ...
 private:
  ...
  float InterpolateAngle(const float time_lower, const float time_upper,
  ...
  float Interpolate(const float time_lower, const float time_upper,
                    const float param_lower, const float param_upper,
                    const float curr_time) const;
};
...
void Trajectory::Impl::KinematicsUpdate(
    TimeSec curr_time, TrajectoryKinematics& traj_kinematics,
    const TimeSec time_at_load, const Kinematics& kinematics_at_load) const {
  ...
  // Env_actor will move towards the first trajectory point before the first
  // time entry. After the last time entry, the env_actor will either stay at
  // the same position or loop back depending on the config params. In between,
  // kinematics is calculated by interpolating between the two relevant values.
  if (curr_time < time_sec_.front() && pose_x_.size() > 0) {
    // Interpolate between last pose and new traj first pose
    float x = Interpolate(time_at_load, time_sec_.front(),
                          kinematics_at_load.pose.position.x(), pose_x_.front(),
                          curr_time);
    float y = Interpolate(time_at_load, time_sec_.front(),
                          kinematics_at_load.pose.position.y(), pose_y_.front(),
                          curr_time);
    float z = Interpolate(time_at_load, time_sec_.front(),
                          kinematics_at_load.pose.position.z(), pose_z_.front(),
                          curr_time);

    auto rpy = TransformUtils::ToRPY(kinematics_at_load.pose.orientation);
    float roll = InterpolateAngle(time_at_load, time_sec_.front(), rpy.x(),
                                  pose_roll_.front(), curr_time);
    float pitch = InterpolateAngle(time_at_load, time_sec_.front(), rpy.y(),
                                   pose_pitch_.front(), curr_time);
    float yaw = InterpolateAngle(time_at_load, time_sec_.front(), rpy.z(),
                                 pose_yaw_.front(), curr_time);

    float x_vel = Interpolate(time_at_load, time_sec_.front(),
                              kinematics_at_load.twist.linear.x(),
                              velocity_linear_x_.front(), curr_time);
    float y_vel = Interpolate(time_at_load, time_sec_.front(),
                              kinematics_at_load.twist.linear.y(),
                              velocity_linear_y_.front(), curr_time);
    float z_vel = Interpolate(time_at_load, time_sec_.front(),
                              kinematics_at_load.twist.linear.z(),
                              velocity_linear_z_.front(), curr_time);
    ...
    float roll =
        InterpolateAngle(time_sec_, pose_roll_, upper, lower, curr_time) +
    ...
    float yaw =
        InterpolateAngle(time_sec_, pose_yaw_, upper, lower, curr_time) +
...
float Trajectory::Impl::InterpolateAngle(float time_lower, float time_upper,
  ...
  float param_val = param_lower_mod + (curr_time - time_lower) *
                                          ((param_upper_mod - param_lower_mod) /
...
float Trajectory::Impl::Interpolate(const std::vector<float>& time_array,
  ...
  return Interpolate(time_lower, time_upper, param_lower, param_upper,
                     curr_time);
}

float Trajectory::Impl::Interpolate(const float time_lower,
                                    const float time_upper,
                                    const float param_lower,
                                    const float param_upper,
                                    const float curr_time) const {
  // edge case where currernt time is either lower than lower time or higher
  // than upper time
