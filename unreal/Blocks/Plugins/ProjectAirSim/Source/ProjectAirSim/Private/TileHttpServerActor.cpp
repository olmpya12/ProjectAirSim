...
bool ATileHttpServerActor::HandleTileRequest(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	if (!ZStr || !XStr || !YStr)
	{
		...
		OnComplete(FHttpServerResponse::Error(EHttpServerResponseCodes::BadRequest, TEXT("BadRequest"), TEXT("Missing path params")));
		return true;
	}
	...
	if (YValue.IsEmpty())
	{
		...
		OnComplete(FHttpServerResponse::Error(EHttpServerResponseCodes::BadRequest, TEXT("BadRequest"), TEXT("Invalid tile y")));
		return true;
	}
	...
	if (Zoom < MinZoom || Zoom > MaxZoom)
	{
		...
		OnComplete(FHttpServerResponse::Error(EHttpServerResponseCodes::NotFound, TEXT("NotFound"), TEXT("Zoom out of range")));
		return true;
	}
	...
	if (!bRenderMissingTiles || !TileCamera)
	{
		...
		OnComplete(FHttpServerResponse::Error(EHttpServerResponseCodes::NotFound, TEXT("NotFound"), TEXT("Tile missing")));
		return true;
	}
	...
	if (!bRendered)
	{
		...
		OnComplete(FHttpServerResponse::Error(EHttpServerResponseCodes::NotFound, TEXT("NotFound"), TEXT("Tile render failed")));
		return true;
	}
