  ...
  if (skeletalMesh == nullptr) {
    if (Id == "Frame") {
      ...
      if (mesh != nullptr) {
        ...
        auto colored_material =
            FString(material_mesh.GetColoredTexture().c_str());
        auto MaterialAsset =
        ...
        for (int32 i = 0; i < NewLink->GetNumMaterials(); ++i) {
          ...
          if (SlotName.ToString().Contains("Carpaint") &&
    ...
    if (Id.find("Wheel") != std::string::npos) {
      ...
      auto simEnvCar =
          static_cast<microsoft::projectairsim::EnvCar&>(SimEnvActor);
      simEnvCar.SetWheelRadius(projectairsim::TransformUtils::ToMeters(
          wheel_radius));  // Set the wheel radius
    }
  } else {
    ...
    auto simEnvCar =
        static_cast<microsoft::projectairsim::EnvCar&>(SimEnvActor);
    simEnvCar.HasSkeletalMesh(
        has_skeletal_mesh);  // Set that the car has a skeletal mesh
    // Scale for EnvCar is set in the skeletal mesh
    {
      auto fvector_scale_local_cur =
          PoseableMeshComponent->GetRelativeScale3D();
      ...
      auto fvector_scale_world_cur =
          PoseableMeshComponent
              ->GetComponentScale();  // Get current world scaling
      float scale_x_local = projectairsim::MathUtils::IsApproximatelyZero(
                                fvector_scale_world_cur.X)
                                ? std::numeric_limits<float>::max()
                                : fvector_scale_world_new.x() /
                                      fvector_scale_world_cur.X *
                                      fvector_scale_local_cur.X;
      float scale_y_local = projectairsim::MathUtils::IsApproximatelyZero(
                                fvector_scale_world_cur.Y)
                                ? std::numeric_limits<float>::max()
                                : fvector_scale_world_new.y() /
                                      fvector_scale_world_cur.Y *
                                      fvector_scale_local_cur.Y;
      float scale_z_local = projectairsim::MathUtils::IsApproximatelyZero(
                                fvector_scale_world_cur.Z)
                                ? std::numeric_limits<float>::max()
                                : fvector_scale_world_new.z() /
                                      fvector_scale_world_cur.Z *
                                      fvector_scale_local_cur.Z;
      // Set the new local scaling
      PoseableMeshComponent->SetRelativeScale3D(
          FVector(scale_x_local, scale_y_local, scale_z_local));
      float MeshBoundsX =
          PoseableMeshComponent->GetSkinnedAsset()->GetBounds().BoxExtent.X;
      float MeshBoundsY =
          PoseableMeshComponent->GetSkinnedAsset()->GetBounds().BoxExtent.Y;
      float MeshBoundsZ =
          PoseableMeshComponent->GetSkinnedAsset()->GetBounds().BoxExtent.Z;
...
void AUnrealEnvActorCar::InitializeSkeletalMesh(USkeletalMesh* skeletalMesh) {
  if (CapsuleComponent == nullptr) {
    CapsuleComponent =
        NewObject<UCapsuleComponent>(this, TEXT("CapsuleComponent"));
    CapsuleComponent->SetupAttachment(
        RootComponent);                     // Attach to the RootComponent
    CapsuleComponent->RegisterComponent();  // Register the component
  }

  PoseableMeshComponent =
      NewObject<UPoseableMeshComponent>(this, TEXT("PoseableMeshComponent"));
  PoseableMeshComponent->SetupAttachment(
      RootComponent);  // Attach to the RootComponent
  PoseableMeshComponent->RegisterComponent();
  PoseableMeshComponent->SetSkinnedAssetAndUpdate(skeletalMesh);
  PoseableMeshComponent->SetCollisionEnabled(
  ...
  for (int32 BoneIndex = 0; BoneIndex < NumBones; BoneIndex++) {
    ...
    if (BoneNameString.Contains(TEXT("FL"))) {
      ...
      FrontLeftWheelLocation = PoseableMeshComponent->GetBoneLocationByName(
          BoneName, EBoneSpaces::ComponentSpace);
      WheelScale = PoseableMeshComponent->GetBoneScaleByName(
          BoneName, EBoneSpaces::ComponentSpace);
    } else if (BoneNameString.Contains(TEXT("FR"))) {
      FrontWheels.push_back(BoneIndex);
    } else if (BoneNameString.Contains(TEXT("BL"))) {
      BackWheels.push_back(BoneIndex);
      BackLeftWheelLocation = PoseableMeshComponent->GetBoneLocationByName(
          BoneName, EBoneSpaces::ComponentSpace);
  ...
  simEnvCar.SetWheelRadius(
      projectairsim::TransformUtils::ToMeters(wheel_radius));

  wheels_distance =
      FVector::Distance(FrontLeftWheelLocation, BackLeftWheelLocation);
  simEnvCar.SetWheelsDistance(
      projectairsim::TransformUtils::ToMeters(wheels_distance));
}
...
void AUnrealEnvActorCar::DetectZPositionwithStaticMesh(float wheel_rad) {
  ...
  FVector endUnder =
      start + FVector(0, 0, -wheel_rad);  // The end of the vector under the car
  FVector endOver =
      start + FVector(0, 0, wheel_rad);  // The end of the vector over the car
  ...
  auto SimEnvActorGrounded =
      static_cast<microsoft::projectairsim::EnvActorGrounded&>(SimEnvActor);
  ...
  if (bHitOver) {
    ...
    SimEnvActorGrounded.SetGroundLevel(
        projectairsim::TransformUtils::NeuToNedLinear(
            projectairsim::TransformUtils::ToMeters(UnrealHelpers::ToVector3(
                CeilingLocation + FVector(0, 0, wheel_rad))))
            .z());
  } else if (bHit) {
    // Floor detected
    ...
    SimEnvActorGrounded.SetGroundLevel(
        projectairsim::TransformUtils::NeuToNedLinear(
            projectairsim::TransformUtils::ToMeters(UnrealHelpers::ToVector3(
                FloorLocation + FVector(0, 0, wheel_rad))))
            .z());
  } else {
    // Floor not detected
    auto ground_level =
        projectairsim::TransformUtils::NeuToNedLinear(
            projectairsim::TransformUtils::ToMeters(
                UnrealHelpers::ToVector3(endUnder + FVector(0, 0, wheel_rad))))
            .z();
...
void AUnrealEnvActorCar::DetectZPositionwithSkeletalMesh(float wheel_rad) {
  ...
  FVector endUnder =
      start + FVector(0, 0, -wheel_rad);  // a trace down to the ground level
  FVector endOver =
      start + FVector(0, 0, wheel_rad);  // a trace up to the ceiling
  ...
  if (PoseableMeshComponent) {
    ...
    for (auto& bwheel : BackWheels) {
      PoseableMeshComponent->SetBoneRotationByName(
          ...
          FRotator(CurrentAngle, 0, 0), EBoneSpaces::ComponentSpace);
    }
  }
}