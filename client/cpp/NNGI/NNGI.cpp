...
NNGI_DECL int nng_dial(nng_socket sid, const char* addr, nng_dialer* dp,
                       int flags) {
  return (::nng_dial(*reinterpret_cast<::nng_socket*>(&sid), addr,
                     reinterpret_cast<::nng_dialer*>(dp), flags));
}

NNGI_DECL void nng_free(void* buf, size_t sz) { return (::nng_free(buf, sz)); }
...
NNGI_DECL const char* nng_strerror(int num) { return (::nng_strerror(num)); }

NNGI_DECL const char* nng_version(void) { return (::nng_version()); }
