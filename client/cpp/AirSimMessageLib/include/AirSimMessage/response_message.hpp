...
// Project AirSim service method calls return a ResponseSuccessMessage on
// success and a ResponseFailureMessage on failure.  There a catch-22 in that
// you can only deserialize the packed reply stream with the correct message
// class but can't determine which message was returned without deserializing it
// first.  Project AirSim clients can use this class to handle that issue, which
// deserializes either message and returns the response information.
class ResponseMessage : public Message {
 public:
  ResponseMessage(const int& id, const nlohmann::json& result,
                  const float& version);
